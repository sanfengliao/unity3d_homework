...
public class FirstController : MonoBehaviour, SceneController, UserAction
{
    ...
    public IActionManager actionManger { get; set; } 
    ...
    private void Update()
    {
        if (currentRound == 3  || actionManger == null)
        {
            return;
        }
        if (actionManger.getDiskNum() == 0 && state == GameState.RUNNING)
        {
            state = GameState.ROUND_FINISH;
        }
        if (actionManger.getDiskNum() ==0 && state == GameState.ROUND_START)
        {
            currentRound = currentRound + 1;
            NextRound();
            actionManger.setDiskNum(10);
    ...
    public void setMode(int i)
    {
        if (i == 0)
        {
          actionManger = gameObject.AddComponent<CCActionManager>();
        }
        else
        {
          actionManger =  gameObject.AddComponent<CCPhysicsManager>();
        }
    }
}
